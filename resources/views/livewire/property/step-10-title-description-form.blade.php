...
{
    #[Validate('required|string|min:300|max:5000')]
     ...
     */
    #[On('propertyGenerateDescription')]
    public function generateDescription()
    {
        ...
        $theProperty = [];
        $theProperty['title']           = $propertyType->name . ' in ' . $propertyAddress->town_city;
        $theProperty['region']          = $propertyAddress->region;
        $theProperty['town']            = $propertyAddress->town_city;
        $theProperty['property_type']   = $propertyType->name;
        $theProperty['plot']            = $this->property->plot;
        $theProperty['plot_description']= $this->property->plot_description;
        $theProperty['area_size']       = $this->property->area_size;
        $theProperty['bedrooms']        = $this->property->bedrooms;
        ...
        if (!empty($propertyAmenities)) {
            foreach ($propertyAmenities as $key => $value) {
                ...
                $amenity = ucwords(str_replace('_', ' ', $key));
                $theProperty['amenities'][$key] = "{$amenity}: {$value}";
            }
        } else {
            $theProperty['amenities'] = null;
        }
        
        $result = json_decode($claudeClient->generateDescription($theProperty));

        $this->description = $result->content[0]->text;
    }
                        ...
                        <div class="flex gap-5">
                            ...
                            <div>                                
                                <button class="flex px-4 py-1 bg-gray-300 hover:bg-gray-400 rounded" 
                                    ...
                                    wire:click="$dispatch('propertyGenerateDescription')"
                                >
                                    <span wire:loading.remove class="flex gap-2 items-center text-sm text-gray-700">
                                    ...
                                    <span wire:loading class="items-center text-sm text-gray-700">
                                        Generating...
                                    </span>
                                </button>
                            </div>
