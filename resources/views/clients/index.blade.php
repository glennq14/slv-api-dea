        ...
        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            ...
            <div>
                <nav aria-label="Pagination" class="inline-flex -space-x-px rounded-md shadow-xs isolate">
                    ...
                    @else
                        <a href="{{ $clients->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 text-gray-400 rounded-l-md inset-ring inset-ring-gray-300 hover:text-gray-700 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
                    ...
                    @foreach ($clients->getUrlRange(1, $clients->lastPage()) as $page => $url)
                        ...
                        <a href="{{ $url }}" {!! $page == $clients->currentPage()
                    ...
                    @if ( $clients->hasMorePages() )
                        <a href="{{ $clients->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 text-gray-400 rounded-r-md inset-ring inset-ring-gray-300 hover:text-gray-700 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
...
<script>
    ...
    function renderClients(limit) {
        ...
        clients.slice(0, limit).forEach(client => {
            if (!client.avatarColor) {
                client.avatarColor = getRandomColor();
            }
    ...
    document.getElementById('showCount').addEventListener('change', (e) => {
        renderClients(parseInt(e.target.value, 10));
    });
